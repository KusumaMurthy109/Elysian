...
import { View, Text, ScrollView, ImageBackground, TouchableOpacity, Pressable, Image, Modal } from "react-native";
...
const UserItineraries = () => {
    ...
    useEffect(() => {
        ...
        const unsubOwned = onSnapshot(qOwned, async (ownedSnap) => {
            const ownedData = ownedSnap.docs.map((doc) => ({ id: doc.id, ...(doc.data() as any) }));
            ...
            result_images.forEach((r) => { imageMap[r.id] = r.img; });

            setUnsplashImageUrl((prev) => ({ ...prev, ...imageMap }));
            setItineraries((prev) => [...ownedData, ...prev.filter((i) => i.ownerId !== currentUser.uid)]);
            setLoading(false);
        });

        const unsubShared = onSnapshot(qShared, async (sharedSnap) => {
            const sharedData = sharedSnap.docs.map((doc) => ({ id: doc.id, ...(doc.data() as any) }));
            ...
            setUnsplashImageUrl((prev) => ({ ...prev, ...imageMap }));
            setItineraries((prev) => [...prev.filter((i) => i.ownerId === currentUser.uid), ...sharedData]);
        });
    ...
    useEffect(() => {
        ...
        const fetchOwnerUsername = async () => {
            const snap = await getDoc(doc(FIREBASE_DB, "users", openItinerary.ownerId));
    ...
    const handleSearchUsers = async (text: string) => {
        ...
        const itinSnap = await getDoc(doc(FIREBASE_DB, "itineraries", selectedItinerary.id));
        ...
        const q1 = query(collection(FIREBASE_DB, "users"), where("username", ">=", lower), where("username", "<=", lower + "\uf8ff"));
        const q2 = query(collection(FIREBASE_DB, "users"), where("username", ">=", upper), where("username", "<=", upper + "\uf8ff"));
    ...
    const addActivityToItinerary = async () => {
        ...
        await updateDoc(itinRef, { activities: [...openItinerary.activities, newActivityObj], updatedAt: new Date() });
        setNewActivity("");
    };
    ...
    const handleToggleLike = async (index: number) => {
        ...
        updatedActivities[index] = {
            ...activity,
            likes: alreadyLiked ? activity.likes.filter(id => id !== currentUser.uid) : [...activity.likes, currentUser.uid],
        };
        await updateDoc(doc(FIREBASE_DB, "itineraries", openItinerary.id), { activities: updatedActivities });
